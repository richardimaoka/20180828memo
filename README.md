...
The hmda-platform project,  which is under the `/hmda` directory, includes HTTP servers and their backend actors. We are not going to cover every single actor in detail, but explain few of them which form the major functionality of the application. At startup, the `HmdaApi` actor brings up four HTTP servers, and also the `HmdaPersistence` actor starts [Cluster Sharding](https://doc.akka.io/docs/akka/current/typed/cluster-sharding.html) for [Akka Persistence](https://doc.akka.io/docs/akka/current/typed/persistence.html). The HTTP servers started by the `HmdaApi` actor are `hmda-filing-api`, `hmda-admin-api` ([API](https://github.com/cfpb/hmda-platform/blob/master/docs/v2/api/admin-api.md#hmda-platform-admin-api)), `hmda-public-api` [(ts](https://github.com/cfpb/hmda-platform/blob/master/docs/v2/api/public-api.md#ts-parsing-and-validation) API, [LAR](https://github.com/cfpb/hmda-platform/blob/master/docs/v2/api/public-api.md#lar-parsing-and-validation) API, [HMDA file](https://github.com/cfpb/hmda-platform/blob/master/docs/v2/api/public-api.md#hmda-file-parsing-and-validation) API ) and `hmda-ws-api`.  The `hmda-admin-api` HTTP server works as an [Event-Sourcing](https://martinfowler.com/eaaDev/EventSourcing.html) style server, where HTTP POST, PUT and DELETE methods are converted into respective commands sent to sharded persistent entity actors started by `HmdaPersistence`, and the persistent entity actor processes the received command then persist the event into [Cassandra](http://cassandra.apache.org/) [journal](https://github.com/akka/akka-persistence-cassandra). After the persistence of the event, the resulting HTTP response is sent back to the client.
